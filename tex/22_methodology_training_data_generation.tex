...
$\sigma$ of peak is calculated similarly as in \cite{Jones2022}, with fano factor calculated using gaussian fitted on accumulated copper plate spectrum - \prettyref{fig:fano-factor}.
...
Although peak in \prettyref{fig:fano-factor} seem to be generated correctly, something is for sure wrong because calculated $FANO$ is about order of magnitude larger than literature says, $NOISE$ is unknown, and constant $0.00358$ is correct only for Silicon Drift Detectors.
Further in the work it will be assumed that $\sigma$ calculation works correctly.

\prettyref{fig:training_data_generation} presents stages of generation of single artificial spectrum. 
\begin{figure}[H] 
  \centering     
...
Example generated spectra with the same elements used (copper and lead) can be seen on \prettyref{fig:sum-spectras}:
