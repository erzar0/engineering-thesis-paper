\documentclass[twoside,a4paper,12pt]{article}
\input{tex/00_preambule.tex}

\author{Eryk Zarębski}
\title{An algorithm for classifying chemical elements using data collected with the X-ray fluorescence technique}
\setDepartment{Faculty of Physics and Applied Computer Science}
\setFaculty{Applied Computer Science}
\setSupervisor{prof. dr hab. inż. Tomasz Szumlak}
\setReviewer{dr inż. TODO}

\begin{document}
\pagestyle{empty}
\maketitle
\null
\newpage

\input{tex/02_student_declaration}
\null
\newpage
\null
\newpage
\include{tex/03_abstract}

\pagestyle{plain}
\linespread{1.3}
\selectfont

\null
\newpage
\pagenumbering{roman}
\tableofcontents
\mbox{}
\null
\newpage

\pagenumbering{arabic}

\include{tex/10_introduction}
\include{tex/20_methodology_used_technologies}
\include{tex/21_methodology_key_algorithms}
\include{tex/22_methodology_training_data_generation}
\include{tex/23_methodology_real_data_preprocessing}
\include{tex/30_results_spectra_clusterization}
\include{tex/31_results_vit_training}
\include{tex/32_results_classifying_spectra}
\include{tex/99_conclusion}

\bibliography{bibliography.bib}
\bibliographystyle{plainnat} 
\end{document}
